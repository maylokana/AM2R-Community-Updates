...
// completely disregard this code if in the respawn state
var respawnCheck = false;
with (oCharacter){
    if (state == RESPAWN) respawnCheck = true;
}
if (!respawnCheck){
    var damage_taken;
    ...
    if (global.playerhealth > 0) with (oCharacter) {
        ...
        if (global.playerhealth <= 0 && state != RESPAWN) {
            alarm[5] = 240;
            state = RESPAWN;
            instance_create(x,y,oTTDeathSequence);
            visible = false;
            statetime = 0;
